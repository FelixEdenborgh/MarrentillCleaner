...
@ScriptManifest(author = "Feed", category = Category.HERBLORE, name = "Marrentill Cleaner", version = 1.0)
                ...
                if(banker.interact("Bank")){
                    if(sleepUntil(() -> getBank().isOpen(), 2000)){
                        if(sleepUntil(() ->getInventory().isFull(), 2000)){
                            getBank().depositAllExcept(item -> item != null && item.getName().contains("Grimy marrentill")); //make so it will deposit all if you have something there
                        }
                        if(sleepUntil(() -> !getInventory().isFull(), 2000)){
                            getBank().withdrawAll("Grimy marrentill");
                            if(sleepUntil(() -> getInventory().isFull(), 2000)){
                                if(getBank().close()){
                                    sleepUntil(() -> !getBank().isOpen(), 2000);
                                    sleep(Calculations.random(300, 3500));
                                    getInventory().interact("Grimy marrentill", "Clean");
